...
impl Vm {
    ...
    pub(crate) fn resolve_virtual_callsite_monomorphic(
    ...
    ) -> Option<std::rc::Rc<super::ResolvedVirtualCallSite>> {
        {
            ...
            if matches!(
                borrow.native,
                NativePayload::Lambda(_) | NativePayload::BytecodeLambda { .. } | NativePayload::RecordMethod { .. }
            ) {
                return None;
            }
        }
    ...
    pub(crate) fn invoke_virtual_native_from_info(
        ...
        if let Some(v) = self.native_virtual(this, &info.class_name, method_name, &info.descriptor, args) {
            if let Some(err) = self.pending_exception_err() {
                return Err(err);
            }
            return Ok(v);
